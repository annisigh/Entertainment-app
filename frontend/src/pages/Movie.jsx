...
const Movie = () => {
	...
	const fetchMovie = useFetchMultiMedia; //Initializing the fetchMovie function using the useFetchMultiMedia custom hook.

	//Initializing the mediaData state variable using useState to store the fetched movie data.
	// State to store fetched media data
	const [mediaData, setMediaData] = useState(null);


	//Using useEffect to fetch movie data when the component mounts:
    // Delaying the fetch by 2000 milliseconds (2 seconds) for demonstration purposes.
    // Calling the fetchMovie function with the setMediaData function as an argument to update the mediaData state variable with the fetched movie data.
    // Including fetchMovie in the dependency array to ensure the effect runs only once after mount.
