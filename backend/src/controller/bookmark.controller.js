...
// Controller func to add a new bookmark.
...
export const deleteBookmarkController = async function (req, res) {
	try {
		...
		// Respond with error message, if an error occur.
		res.status(500).json({
			success: false,
...
export const fetchUserBookmarkController = async function (req, res) {
	try {
		...
		// Find bookmarks associated with the user and exclude as fields (_id, __v, user).
